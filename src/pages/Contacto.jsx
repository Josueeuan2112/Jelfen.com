...
const redes = [
  {
    nombre: 'Instagram',
    url: 'https://www.instagram.com/jelfen_mid?utm_source=ig_web_button_share_sheet&igsh=ZDNlZDc0MzIxNw==',
    activo: true,
    svg: (
...
function HeroContacto() {
  return (
    <section className="bg-sky-50 py-20 px-6 text-center text-white">
      <span className="bg-gray-800 text-white text-xs font-bold px-4 py-2 rounded-full tracking-widest uppercase mb-6 inline-block">
        Estamos aquí para ayudarte
      </span>
      <h1 className="text-gray-800 text-5xl font-bold mb-4">Contáctanos</h1>
      <p className="text-gray-600 text-xl max-w-xl mx-auto">
        ¿Tienes dudas, sugerencias o quieres saber más sobre Jelfen? Escríbenos — respondemos en menos de 24 horas.
      </p>
    </section>
  )
}
  ...
  return (
    <section className="py-12 px-6 bg-sky-50">
      <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-5">
        {info.map((item, i) => (
          <div key={i} className={`${item.color} border-2 rounded-2xl p-6 text-center`}>
            <div className="text-4xl mb-3">{item.icono}</div>
            <h3 className="font-bold text-gray-800 mb-1">{item.titulo}</h3>
            <p className="text-sky-600 font-semibold">{item.detalle}</p>
...
function Formulario() {
  ...
  const asuntos = [
    'Tengo una duda como paciente',
    'Quiero registrarme como médico',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!form.nombre || !form.correo || !form.mensaje || !form.asunto) return
    setEstado('enviando')
